...
sequelize
  ...
  .then(async () => {
    console.log("✅ Database connected successfully.");
    try {
      // 1. Fix Events ID Sequence
      await sequelize.query(
        "SELECT setval('events_id_seq', (SELECT MAX(id) FROM events))"
      );

      // 🔥 2. FIX USERS ID SEQUENCE (Added this to fix your error)
      // This tells Postgres: "Look at the highest ID in 'users', and set the counter to that number."
      await sequelize.query(
